...
import { setNewsExperience } from "../redux/actions/newsActions"
...
const TourExperience = () => {
  ...
  const news = useSelector((state) => state.newsExperience.news)
  ...
  useEffect(() => {
    const ListTour = async () => {
      const response = await axios
        .get(`${urlNews}/getbyexperience?_limit=10`, {
          headers: {
            "api-key": "duy",
          },
        })
        .catch((err) => console.log("err", err))
      dispatch(setNewsExperience(response.data))
    }
    ListTour()
  }, [dispatch])
  ...
  return (
    <Container>
      <Title>Trải nghiệm du lịch</Title>
      <Wrapper>

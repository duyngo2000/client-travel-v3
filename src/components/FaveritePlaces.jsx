...
import { setProductFaverite } from "../redux/actions/productActions"
...
const FaveritePlaces = () => {
  const language = useSelector((state) => state.setLanguage.language)
  const products = useSelector(
    (state) => state.faveriteProduct.faveriteProducts
  )
  ...
  useEffect(() => {
    const ListTour = async () => {
      const response = await axios
        .get(`${urlTour}/getbyfaverite?_limit=8`, {
          headers: {
            "api-key": "duy",
          },
        })
        .catch((err) => console.log("err", err))
      dispatch(setProductFaverite(response.data))
    }
    ListTour()
  }, [dispatch])
  ...
  return (
    <Container>
      <Title>
        {language === "VN" ? "Địa danh nổi tiếng" : "Famous places"}
      </Title>
      <Wrapper>
        {products !== undefined &&
          products.map(

...
import { setProductExpensive } from "../redux/actions/productActions"
...
const ExpensiveTour = () => {
  ...
  const products = useSelector(
    (state) => state.expensiveProduct.expensiveProducts
  )
  ...
  useEffect(() => {
    const ListTour = async () => {
      const response = await axios
        .get(`${urlTour}/getbyexpensivetour?_limit=8`, {
          headers: {
            "api-key": "duy",
          },
        })
        .catch((err) => console.log("err", err))
      dispatch(setProductExpensive(response.data))
    }
    ListTour()
  }, [dispatch])
  ...
  return (
    <Container>
      <Title>{language === "VN" ? "Tour dài hạn" : "long-term tour"}</Title>
      <Wrapper>
        {products !== undefined &&
          products.map(
            (item, index) =>
